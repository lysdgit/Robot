...
dh_params = [
    RevoluteDH(theta=0, d=495.5, a=680.0, alpha=-np.pi/2),
    RevoluteDH(theta=0, d=0, a=0, alpha=np.pi/2),
    RevoluteDH(theta=0, d=680, a=0, alpha=-np.pi/2),
    RevoluteDH(theta=0, d=0, a=0, alpha=np.pi/2),
    RevoluteDH(theta=0, d=0, a=0, alpha=-np.pi/2),
    RevoluteDH(theta=0, d=284.5, a=0, alpha=0)
]
robot = DHRobot(dh_params)

robot.joint_config = [
    {"type": "R", "qlim": [-np.pi, np.pi]},  # 关节类型为旋转关节，限制为[-pi, pi]
    {"type": "P", "qlim": [0, 10]}  # 关节类型为平移关节，限制为[0, 10]
]

robot.save("MYROBOT.mat")
